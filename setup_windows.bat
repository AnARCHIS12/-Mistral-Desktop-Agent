...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "$lines=@('MISTRAL_API_KEY='+$m,'MISTRAL_MODEL=mistral-large-latest','MISTRAL_MIN_SECONDS_BETWEEN_CALLS=20','MISTRAL_RATE_LIMIT_BACKOFF_SECONDS=60','TELEGRAM_BOT_TOKEN='+$t,'ENABLE_TELEGRAM='+$e,'HOST=0.0.0.0','PORT='+$p,'DATABASE_PATH=data/agent_memory.sqlite3','SCREENSHOT_PATH=data/latest_screenshot.png','SCREENSHOT_BACKEND=auto','FILE_ACCESS_MODE=full','ALLOWED_FILE_ROOTS=','TERMINAL_WORKDIR='+$home,'SEARCH_ENGINE=duckduckgo','MAX_STEPS=50','MAX_RETRIES=3','LOOP_DELAY_SECONDS=1.0','MAX_REPEATED_ACTIONS=3','TERMINAL_TIMEOUT_SECONDS=30','BROWSER_HEADLESS=false');" ^
